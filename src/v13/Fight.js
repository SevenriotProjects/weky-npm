...
const db = new Datastore({ filename: 'fightdb', autoload: true });
					...
					if (!checkHealth(player)) {
						...
						if (checkHealth(player)) {
							...
							if (options.returnWinner) {
								...
								db.insert({ _id: `Fight_${options.message.guild.id}_${options.gameID}`, memberId: gameData[tempPlayer].member.id }, (err, newDoc) => {});
							}
							question.edit({
								embeds: [__embed],
								components: [row],
