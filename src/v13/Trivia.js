...
const db = new Datastore({ filename: 'triviadb', autoload: true });
	...
	gameCollector.on('collect', async (trivia) => {
		...
		if (trivia.customId === winningID) {
			...
			if (options.returnWinner) {
				...
				db.insert({ _id: `Trivia_${options.message.guild.id}_${options.gameID}`, authorId: options.message.author.id });
