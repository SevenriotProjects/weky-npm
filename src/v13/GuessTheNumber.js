...
const db = new Datastore({ filename: 'gtnum', autoload: true });
const currentGames = new Object();
const Discord = require('discord.js');
const functions = require('../../functions/function');

module.exports = async (options) => {
	const key = `GuessTheNumber_${options.message.guild.id}_${options.gameID}`;
		...
		collector.on('collect', async (_msg) => {
			...
			if (parsedNumber === options.number) {
				...
				if (options.returnWinner) {
					...
					db.insert({ _id: `GuessTheNumber_${options.message.guild.id}_${options.gameID}`, authorId: options.message.author.id }, (err, newDoc) => {});
				}
			}
